...
export default class SearchExampleStandard extends Component {
    ...
    handleSubmit = () => {
        if (this.props.query) {
            ...
            client({method: 'GET', path: '/search?query=' + encodeURIComponent(this.props.query)}).then(response => {
                console.log(response);
                this.setState({isLoading: false});
                this.props.updateResults(response.entity);
            }).catch(error => {
    ...
    handleRate = (index, id, e, {rating}) => {
        this.props.updateRate(index, rating);
                            ...
                            <Modal key={i} trigger={
                                <List.Item key={i}>
                                    <List.Content floated='right'>
                                        ...
                                        <Label color={color[Math.round(res.avg_rating)]}>
                                            <Icon name='star'/>{(Math.round(res.avg_rating * 100) / 100).toFixed(2)}
                                        </Label>
                                    </List.Content>
                                ...
                                <Modal.Actions>
                                    <Rating disabled={!this.props.canRate} maxRating={5} rating={res.user_rating}
                                            icon='star' size='large' onRate={this.handleRate.bind(this, i, res.id)}/>
                                </Modal.Actions>
                            </Modal>
                        );
                    })}
                </List> : 'No Recipe Found'}
        </Segment>
    ...
    render() {
        return (
            <Grid>
                <Grid.Column width={6}>
                    <Input
                        action={{color: 'blue', content: 'Search', onClick: () => this.handleSubmit()}}
                        icon='search'
                        iconPosition='left'
                        placeholder='Search...'
